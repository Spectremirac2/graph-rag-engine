...
Correctness is verified by a seeded **property / differential test suite** graded
as hard as the code (see *Verification* below), not by a single smoke test.
...
- It is a solo, from-scratch, oracle-verified implementation of **known
  techniques** (Label Propagation, modularity, BFS) with their limitations stated
  honestly. It is **not** "world-first", state-of-the-art, or production-grade.
...
- `src/graph.rs` — `KnowledgeGraph`: entity/relationship structs, undirected
...
- `src/indexer.rs` — `GraphRagIndexer`: structured-line parser and the local/global
  query router.
